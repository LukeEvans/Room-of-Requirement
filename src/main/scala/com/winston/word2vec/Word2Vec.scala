...
class Word2Vec {
  ...
  def load(filename: String, limit: Integer = Int.MaxValue, normalize: Boolean = true): Unit = {
    ...
    var vector = new Array[Float](vecSize) 
    
    var normFactor = 1f
    for (j <- 0 until math.min(numWords, limit)) {
      ...
      mongo.insert(MongoDBObject("word" -> word, "vector" -> vector.map(_ / normFactor))) 
      

      // Eat up the next delimiter character
      //reader.read() // Remove for google-vector.bin
  ...
  def contains(word:String): Boolean = {
    if(mongo.findOneSimple("word", word) != null)
      return true
    else
      false
  }
...
//  }
  ...
  /** Find the Cosine Distance between two word vectors
   * 
   */
  
  def dist(word1:String, word2:String):(String, String, Double) = {
    
    val wordVector1 = vector(word1) //vocab.get(word1)
    val wordVector2 = vector(word2)
    
    if(wordVector1 == null || wordVector2 == null){
      println("Error loading vectors")
      null
    }
    
    val dist = cosine(wordVector1, wordVector2)
